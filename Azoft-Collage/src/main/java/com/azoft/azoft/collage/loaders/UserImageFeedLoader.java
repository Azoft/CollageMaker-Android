...
import com.azoft.azoft.collage.data.Image;
...
public class UserImageFeedLoader extends DataAsyncTaskLibLoader<UserRecentFeedResult> {
    ...
    public UserImageFeedLoader(final Context context, final String userId, final String accessToken) {
        super(context);
        ...
        for (int i = 0; i < TRIES_COUNT; ++i) {
            try {
                ...
                if (null == currentMaxId) {
                    response = ServerConnector.getInstagramService().getUserRecentFeed(mUserId, FEED_COUNT, mAccessToken);
                } else {
                    response = ServerConnector.getInstagramService().getUserRecentFeed(mUserId, FEED_COUNT, currentMaxId, mAccessToken);
                }
                if (null != response) {
                    return response;
                }
